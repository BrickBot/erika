...
;;; 
;;; CVS: $Id: hal.s,v 1.1.1.1 2003/02/21 09:53:07 pj Exp $
...
.ifdef __MULTI__	
	...
	.macro M_swap_stack         
         ;; r0=tid of new thread (NIL=-1 for dummy);
	 ;; each element of vector h8_thread_stack_id is 1 byte;
	 ;; so we obtain r1=h8_thread_stack_id[r0+1] just with this mov:
	 
	 mov.b	@(_ERIKA_h8_thread_stack_id+1,r0), r1l
         mov.b  @_ERIKA_h8_active_stack_id, r2l
	 cmp.b	r1l, r2l
	 beq	lMss\@	
	 mov.b  #0, r2h
	 add.w  r2, r2
	 mov.w	sp, @(_ERIKA_h8_sp, r2)          ; h8_sp[h8_active_thread_id]=sp
	 mov.b  r1l, @_ERIKA_h8_active_stack_id  ; h8_active_stack_id=h8_thread_stack_id[r0+1]
	 mov.b  #0, r1h
	 add.w  r1, r1
	 mov	@(_ERIKA_h8_sp, r1), sp          ; sp=h8_sp[h8_active_stack_id];
lMss\@:
	.endm
.endif
	...
	;;; end_instance
	.ifdef __FP__
	    mov.w #_ERIKA_fp_thread_end_instance, r1
	.endif

	.ifdef __SRPT__
	    mov.w #_ERIKA_srpt_thread_end_instance, r1
	.endif
	 ...
	 ; => r0 = h8_thread_body[r0] = @(_h8_thread_body, r0*2)
	 ; Note that r0*2 is needed since each vector element is 2 byte long
	 add     r0, r0          ; r0=r0*2
	 mov.w	@(_ERIKA_h8_thread_body, r0), r0
...
;;; ****************
	.section .data
	.global  _ERIKA_IRQ_nesting_level
	
	.section .text
	.global _ERIKA_h8_hal_ready2stacked	
	.global _ERIKA_h8_hal_endcycle_ready
	.global _ERIKA_h8_hal_endcycle_stacked
	.global _ERIKA_h8_hal_IRQ_ready
	.global _ERIKA_h8_hal_IRQ_stacked
.ifdef __MULTI__
	.global _ERIKA_h8_hal_stkchange
.endif	

;;; ****************
;;; utility function
;;; ****************
	.global _ERIKA_h8_hal_reboot
		
	.align 1
_ERIKA_h8_hal_ready2stacked:
...
;;; ***************************************************************************
	...
	M_start_thread
        ...
        .align 1
_ERIKA_h8_hal_endcycle_stacked:	
	mov	r6, sp
...
;;; ********************************************************************************
	;; clear ERIKA_IRQ_nesting_level
	mov	#0, r0                             
	mov	r0, @_ERIKA_IRQ_nesting_level	 
