 ...
 *
 * CVS: $Id: ErikaOnOffButton.c,v 1.1 2004/06/01 16:42:37 vestrini Exp $
 */

#define S_FUNCTION_NAME  ErikaOnOffButton
...
boolean_T use_interrupt(SimStruct *S);
/* returns block input */
...
static void mdlCheckParameters(SimStruct *S)
{
  ...
  if ( mxIsComplex(ssGetSFcnParam(S,1)) ||
    ...
    ssSetErrorStatus(S, "Second parameter (use interrupt) has to be a non "
		     "complex double type, with 2 dimensions and one element"
		     "with value 0 (no) or  other (yes)");
    return;
  }
}
...
static void mdlInitializeSizes(SimStruct *S)
{
  ...
  if (ssGetNumSFcnParams(S) == ssGetSFcnParamsCount(S)) {
    ...
    ssSetErrorStatus(S, "2 Parameters expected: "
		     "output mode and use interrupt");
    return; 
  }
...
{
  boolean_T op, ot, oe, ui;
  ui = use_interrupt(S);
  op = output_mode_pass(S);
  ot = output_mode_trigger(S);
  oe = output_mode_edge(S);

  ssWriteRTWScalarParam(S, "UseInterrupt", &ui, SS_BOOLEAN);
  ...
  if (use_interrupt(S)) {
    ssWriteRTWStrParam(S, "ErikaInterrupt", "OnOffButton");
    ssWriteRTWStrParam(S, "InterruptHandler", "onoffbutton_handler"); 
    ssWriteRTWStrParam(S, "IRQ", "ONOFFBUTTON_IRQ");
  }
...
boolean_T use_interrupt(SimStruct *S) 
{
  return ( *((real_T*) mxGetPr(ssGetSFcnParam(S,1))) == 1);
}
